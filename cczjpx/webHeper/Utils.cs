...
namespace cczjpx
{
    public class Utils
    {
        ...
        /// <returns></returns>
        public static XElement CreateXML<T>(T entity)
        {
            XElement xel = new XElement("Param");
            foreach (var item in entity.GetType().GetProperties())
            {
                if (item.GetValue(entity, null) != null)
                {
                    if (item.GetValue(entity, null) != null)
                        xel.SetAttributeValue(item.Name, item.GetValue(entity, null));
                }
            }
            return xel;
        }
        ...
        /// <param name="dr">一条数据</param>
        /// <param name="entity">实体类型</param>
        /// <returns>返回赋值后的实体</returns>
        public static T ReceiveEntity<T>(DataRow dr, T entity)
        {
            T baseEntity = (T)Activator.CreateInstance(entity.GetType());
            foreach (PropertyInfo propertyInfo in entity.GetType().GetProperties())
            {
                try { baseEntity.GetType().GetProperty(propertyInfo.Name).SetValue(baseEntity, dr[propertyInfo.Name], null); }
                catch { }
            }
            return baseEntity;
        }
        ...
        public static List<T> ReceiveEntityList<T>(DataTable table, T entity) where T : new()
        {
            List<T> rlist = new List<T>();
            T baseEntity = default(T);
        ...
        /// <param name="c">当前提交的表单</param>
        /// <param name="entity"></param>
        public static void SetEntity<T>(Control c, ref T entity)
        {
            foreach (var item in c.Controls)
            {
                if (item is TextBox)
                {
                    ...
                    SetValueDate(t.ToolTip, t.Text, ref entity);

                }
                else if (item is DropDownList)
                {
                    DropDownList d = item as DropDownList;
                    SetValueDate(d.ToolTip, d.SelectedValue, ref entity);
                }
                else if (item is HtmlTextArea)
                {
                    HtmlTextArea t = item as HtmlTextArea;
                    SetValueDate(t.Attributes["title"], t.InnerHtml, ref entity);
                }
        ...
        protected static void SetValueDate<T>(string pName, string value, ref T entity)
        {
            if (pName.Contains("_"))
            {
                switch (pName.Split('_')[0].ToLower())
                {
                    case "i":
                        ...
                        entity.GetType().GetProperty(pName.Split('_')[1]).SetValue(entity, Convert.ToInt32(value), null);
                        break;
                    case "s":
                        entity.GetType().GetProperty(pName.Split('_')[1]).SetValue(entity, Convert.ToString(value), null);
                        break;
                }
            }
        ...
        /// </summary>
        /// <typeparam name="T">实体</typeparam>
        /// <param name="c">提交的form表单</param>
        /// <param name="entity">实体对象</param>
        public static void SetControlsValue<T>(Control c, T entity)
        {
            string conName = null;
            foreach (Control item in c.Controls)
            {
                conName = item.ID;
                // object o = entity.GetType().GetProperty(conName.Substring(3, conName.Length - 3));
                if (conName != null && entity.GetType().GetProperty(conName.Substring(3, conName.Length - 3)) != null)
                {
                    SetControlValue(item, entity);
                }
            }
        }
        /// <summary>
        ...
        /// <param name="entity">实体对象</param>
        protected static void SetControlValue<T>(Control c, T entity)
        {
            string colName = c.ID;
            string colValue = entity.GetType().GetProperty(colName.Substring(3, colName.Length - 3)).GetValue(entity, null).ToString();
            switch (c.GetType().ToString())
            {
                case "System.Web.UI.WebControls.TextBox":
                    (c as TextBox).Text = colValue;
                    break;
                case "System.Web.UI.WebControls.DropDownList":
                    DropDownList d = (c as DropDownList);
                    SetDropDownListSelected(colValue, d);
                    break;
                case "System.Web.UI.WebControls.Label":
                    (c as Label).Text = colValue;
                    break;
                case "System.Web.UI.HtmlControls.HtmlTextArea":
                    (c as HtmlTextArea).InnerHtml = colValue;
                    break;
        ...
        /// <param name="d"></param>
        public static void SetDropDownListSelected(string selectValue, DropDownList d)
        {
            int size = d.Items.Count - 1;
            for (int i = 0; i < size; i++)
            {
                if (d.Items[i].Value == selectValue)
                {
                    d.SelectedIndex = i;
                    break;
                }
